...
@Component({ template: '' })
export abstract class FactureBonComponent extends CLFDocComponent implements OnInit, OnDestroy {

    get titre(): string {
        return `${this.utile.texte.facture.def.Bon}${this.clfDoc.no !== 0
            ? ' n° ' + this.clfDoc.no
    ...
    créeBarreTitre = (): IBarreTitre => {
        const groupe = Fabrique.titrePage.groupeRetour(this.utile.lien.retourDeBon(this.clfDoc));
