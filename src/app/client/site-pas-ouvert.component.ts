...
export class SitePasOuvertComponent extends PageBaseComponent implements OnInit {
    ...
    rafraichit(aEtéRedirigé?: boolean, préparationEnCours?: boolean) {
        ...
        this.étiquetteAction.fixeContenus(this.contexte.site.ouvert
    ...
    ngOnInit() {
        ...
        this.subscriptions.push(
            this.route.data.subscribe(data => {
                ...
                const index = segments.findIndex(s => s === ClientPages.commandes.path);
                if (index !== -1) {
                    aEtéRedirigé = true;
                    préparationEnCours = index + 1 < segments.length - 1 && segments[index + 1] === CommandePages.nouveau.path;
                }
